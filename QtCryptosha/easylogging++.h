...
namespace base {\
namespace utils {\
std::string s_currentUser = element_names::base::utils::OS::currentUser(); \
std::string s_currentHost = element_names::base::utils::OS::currentHost(); \
bool s_termSupportsColor = element_names::base::utils::OS::termSupportsColor(); \
}\
}\
}
...
public:
    ...
    virtual void log(element_names::base::type::ostream_t&) const = 0;
private:
    friend inline element_names::base::type::ostream_t& operator<<(element_names::base::type::ostream_t& os, const Loggable& loggable) {
        loggable.log(os);
        return os;
    }
};
namespace base {
    ...
    public:
        ...
        virtual void log(element_names::base::type::ostream_t& os) const {
            os << m_format;
        }
    ...
    private:
        ...
        friend class element_names::Logger;  // To resolve loggerId format specifier easily
    };
}  // namespace base
...
public:
    ...
    virtual inline void log(element_names::base::type::ostream_t& os) const {
...
public:
    ...
    private:
        friend class element_names::Loggers;
...
namespace base {
    ...
    private:
        ...
        friend class element_names::Helpers;
        friend class element_names::base::MessageBuilder;
        friend class element_names::base::Writer;
        friend class element_names::base::DefaultLogDispatchCallback;
        friend class element_names::base::LogDispatcher;
...
#   undef LOGGER_LEVEL_WRITERS_SIGNATURES
    ...
    private:
        ...
        friend class element_names::LogMessage;
        friend class element_names::Loggers;
        friend class element_names::Helpers;
        friend class element_names::base::RegisteredLoggers;
        friend class element_names::base::DefaultLogDispatchCallback;
        friend class element_names::base::MessageBuilder;
        friend class element_names::base::Writer;
        friend class element_names::base::PErrorWriter;
        friend class element_names::base::Storage;
        friend class element_names::base::PerformanceTracker;
        friend class element_names::base::LogDispatcher;
    ...
    namespace base {
        ...
        private:
            ...
            friend class element_names::Helpers;
            friend class element_names::base::DefaultLogDispatchCallback;
            friend class element_names::LogBuilder;
            friend class element_names::base::MessageBuilder;
            friend class element_names::base::Writer;
            friend class element_names::base::PerformanceTracker;
            friend class element_names::base::LogDispatcher;
...
#define ELPP element_names::base::elStorage
...
element_names::base::type::ostream_t& operator<<(element_names::base::type::ostream_t& ss, const ContainerType& container) {\
const element_names::base::type::char_t* sep = ELPP->hasFlag(element_names::LoggingFlag::NewLineForContainer) ? \
ELPP_LITERAL("\n    ") : ELPP_LITERAL(", ");\
...
ss << ELPP_LITERAL("[");\
for (std::size_t i = 0; elem != endElem && i < element_names::base::consts::kMaxLogPerContainer; ++i, ++elem) { \
ss << ElementInstance;\
...
#   if ELPP_VERBOSE_LOG
    ...
    inline void Logger::verbose(int vlevel, const char* s, const T& value, const Args&... args) {
        base::threading::ScopedLock scopedLock(lock());
        log_(element_names::Level::Verbose, vlevel, s, value, args...);
    }
    template <typename T>
    inline void Logger::verbose(int vlevel, const T& log) {
        base::threading::ScopedLock scopedLock(lock());
        log_(element_names::Level::Verbose, vlevel, log);
    }
#   else
    template <typename T, typename... Args>
...
#else
#   define ELPP_CURR_FILE_PERFORMANCE_LOGGER element_names::base::consts::kPerformanceLoggerId
#endif
    class PerformanceTrackingData {
    public:
    ...
    namespace base {
        ...
        public:
            ...
            m_level(level), m_hasChecked(false), m_lastCheckpointId(std::string()), m_enabled(false) {
                ...
                element_names::Logger* loggerPtr = ELPP->registeredLoggers()->get(loggerId, false);
        ...
        private:
            ...
            friend class element_names::PerformanceTrackingData;
        ...
        protected:
            void handle(const PerformanceTrackingData* data) {
                ...
                element_names::base::Writer(m_data->performanceTracker()->level(), m_data->file(), m_data->line(), m_data->func()).construct(1, m_data->loggerId().c_str()) << ss.str();
            }
        private:
            ...
            static std::string crashReason(int sig) {
                ...
                for (int i = 0; i < base::consts::kCrashSignalsCount; ++i) {
                    if (base::consts::kCrashSignals[i].numb == sig) {
                        ...
                        if (ELPP->hasFlag(element_names::LoggingFlag::LogDetailedCrashReason)) {
                            ss << std::endl <<
...
#endif  // ELPP_STACKTRACE
                ELPP_WRITE_LOG(element_names::base::Writer, level, base::DispatchAction::NormalLog, logger) << ss.str();
            }
...
element_names::base::type::ostream_t& operator<<(element_names::base::type::ostream_t& OutputStreamInstance, const ClassType& ClassInstance)
...
#define ELPP_INITIALIZE_SYSLOG(id, opt, fac) element_names::SysLogInitializer elSyslogInit(id, opt, fac)
    ...
    public:
        ...
        static inline void setCrashHandler(const element_names::base::debug::CrashHandler::Handler& crashHandler) {
            element_names::elCrashHandler.setHandler(crashHandler);
        }
        ...
        static inline void logCrashReason(int sig, bool stackTraceIfAvailable = false,
            ...
            element_names::base::debug::logCrashReason(sig, stackTraceIfAvailable, level, logger);
        }
        ...
        static std::string convertTemplateToStdString(const T& templ) {
            element_names::Logger* logger = 
            ELPP->registeredLoggers()->get(element_names::base::consts::kDefaultLoggerId);
            if (logger == nullptr) {
                return std::string();
            }
            base::MessageBuilder b;
        ...
        static inline const element_names::base::utils::CommandLineArgs* commandLineArgs(void) {
...
#   define ELPP_MIN_UNIT element_names::base::TimestampUnit::Microsecond
#else
#   define ELPP_MIN_UNIT element_names::base::TimestampUnit::Millisecond
...
#define TIMED_SCOPE(obj, blockname) element_names::base::PerformanceTracker obj(blockname, ELPP_MIN_UNIT)
#define TIMED_BLOCK(obj, blockName) for (struct { int i; element_names::base::PerformanceTracker timer; } obj = { 0, \
element_names::base::PerformanceTracker(blockName, ELPP_MIN_UNIT) }; obj.i < 1; ++obj.i)
...
// Normal logs
#if ELPP_INFO_LOG
#   define CINFO(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, element_names::Level::Info, dispatchAction, __VA_ARGS__)
#else
#   define CINFO(writer, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_INFO_LOG
#if ELPP_WARNING_LOG
#   define CWARNING(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, element_names::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#   define CWARNING(writer, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_WARNING_LOG
#if ELPP_DEBUG_LOG
#   define CDEBUG(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, element_names::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#   define CDEBUG(writer, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_DEBUG_LOG
#if ELPP_ERROR_LOG
#   define CERROR(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, element_names::Level::Error, dispatchAction, __VA_ARGS__)
#else
#   define CERROR(writer, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_ERROR_LOG
#if ELPP_FATAL_LOG
#   define CFATAL(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, element_names::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#   define CFATAL(writer, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_FATAL_LOG
#if ELPP_TRACE_LOG
#   define CTRACE(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, element_names::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE(writer, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
#   define CVERBOSE(writer, vlevel, dispatchAction, ...) if (VLOG_IS_ON(vlevel)) writer(\
element_names::Level::Verbose, __FILE__, __LINE__, ELPP_FUNC, dispatchAction, vlevel).construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#else
#   define CVERBOSE(writer, vlevel, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_VERBOSE_LOG
// Conditional logs
#if ELPP_INFO_LOG
#   define CINFO_IF(writer, condition_, dispatchAction, ...) \
ELPP_WRITE_LOG_IF(writer, (condition_), element_names::Level::Info, dispatchAction, __VA_ARGS__)
#else
#   define CINFO_IF(writer, condition_, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_INFO_LOG
#if ELPP_WARNING_LOG
#   define CWARNING_IF(writer, condition_, dispatchAction, ...)\
ELPP_WRITE_LOG_IF(writer, (condition_), element_names::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#   define CWARNING_IF(writer, condition_, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_WARNING_LOG
#if ELPP_DEBUG_LOG
#   define CDEBUG_IF(writer, condition_, dispatchAction, ...)\
ELPP_WRITE_LOG_IF(writer, (condition_), element_names::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#   define CDEBUG_IF(writer, condition_, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_DEBUG_LOG
#if ELPP_ERROR_LOG
#   define CERROR_IF(writer, condition_, dispatchAction, ...)\
ELPP_WRITE_LOG_IF(writer, (condition_), element_names::Level::Error, dispatchAction, __VA_ARGS__)
#else
#   define CERROR_IF(writer, condition_, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_ERROR_LOG
#if ELPP_FATAL_LOG
#   define CFATAL_IF(writer, condition_, dispatchAction, ...)\
ELPP_WRITE_LOG_IF(writer, (condition_), element_names::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#   define CFATAL_IF(writer, condition_, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_FATAL_LOG
#if ELPP_TRACE_LOG
#   define CTRACE_IF(writer, condition_, dispatchAction, ...)\
ELPP_WRITE_LOG_IF(writer, (condition_), element_names::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_IF(writer, condition_, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
#   define CVERBOSE_IF(writer, condition_, vlevel, dispatchAction, ...) if (VLOG_IS_ON(vlevel) && (condition_)) writer( \
element_names::Level::Verbose, __FILE__, __LINE__, ELPP_FUNC, dispatchAction, vlevel).construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#else
#   define CVERBOSE_IF(writer, condition_, vlevel, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_VERBOSE_LOG
// Occasional logs
#if ELPP_INFO_LOG
#   define CINFO_EVERY_N(writer, occasion, dispatchAction, ...)\
ELPP_WRITE_LOG_EVERY_N(writer, occasion, element_names::Level::Info, dispatchAction, __VA_ARGS__)
#else
#   define CINFO_EVERY_N(writer, occasion, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_INFO_LOG
#if ELPP_WARNING_LOG
#   define CWARNING_EVERY_N(writer, occasion, dispatchAction, ...)\
ELPP_WRITE_LOG_EVERY_N(writer, occasion, element_names::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#   define CWARNING_EVERY_N(writer, occasion, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_WARNING_LOG
#if ELPP_DEBUG_LOG
#   define CDEBUG_EVERY_N(writer, occasion, dispatchAction, ...)\
ELPP_WRITE_LOG_EVERY_N(writer, occasion, element_names::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#   define CDEBUG_EVERY_N(writer, occasion, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_DEBUG_LOG
#if ELPP_ERROR_LOG
#   define CERROR_EVERY_N(writer, occasion, dispatchAction, ...)\
ELPP_WRITE_LOG_EVERY_N(writer, occasion, element_names::Level::Error, dispatchAction, __VA_ARGS__)
#else
#   define CERROR_EVERY_N(writer, occasion, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_ERROR_LOG
#if ELPP_FATAL_LOG
#   define CFATAL_EVERY_N(writer, occasion, dispatchAction, ...)\
ELPP_WRITE_LOG_EVERY_N(writer, occasion, element_names::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#   define CFATAL_EVERY_N(writer, occasion, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_FATAL_LOG
#if ELPP_TRACE_LOG
#   define CTRACE_EVERY_N(writer, occasion, dispatchAction, ...)\
ELPP_WRITE_LOG_EVERY_N(writer, occasion, element_names::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_EVERY_N(writer, occasion, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
...
#else
#   define CVERBOSE_EVERY_N(writer, occasion, vlevel, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_VERBOSE_LOG
// After N logs
#if ELPP_INFO_LOG
#   define CINFO_AFTER_N(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_AFTER_N(writer, n, element_names::Level::Info, dispatchAction, __VA_ARGS__)
#else
#   define CINFO_AFTER_N(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_INFO_LOG
#if ELPP_WARNING_LOG
#   define CWARNING_AFTER_N(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_AFTER_N(writer, n, element_names::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#   define CWARNING_AFTER_N(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_WARNING_LOG
#if ELPP_DEBUG_LOG
#   define CDEBUG_AFTER_N(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_AFTER_N(writer, n, element_names::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#   define CDEBUG_AFTER_N(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_DEBUG_LOG
#if ELPP_ERROR_LOG
#   define CERROR_AFTER_N(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_AFTER_N(writer, n, element_names::Level::Error, dispatchAction, __VA_ARGS__)
#else
#   define CERROR_AFTER_N(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_ERROR_LOG
#if ELPP_FATAL_LOG
#   define CFATAL_AFTER_N(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_AFTER_N(writer, n, element_names::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#   define CFATAL_AFTER_N(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_FATAL_LOG
#if ELPP_TRACE_LOG
#   define CTRACE_AFTER_N(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_AFTER_N(writer, n, element_names::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_AFTER_N(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
...
#else
#   define CVERBOSE_AFTER_N(writer, n, vlevel, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_VERBOSE_LOG
// N Times logs
#if ELPP_INFO_LOG
#   define CINFO_N_TIMES(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_N_TIMES(writer, n, element_names::Level::Info, dispatchAction, __VA_ARGS__)
#else
#   define CINFO_N_TIMES(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_INFO_LOG
#if ELPP_WARNING_LOG
#   define CWARNING_N_TIMES(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_N_TIMES(writer, n, element_names::Level::Warning, dispatchAction, __VA_ARGS__)
#else
#   define CWARNING_N_TIMES(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_WARNING_LOG
#if ELPP_DEBUG_LOG
#   define CDEBUG_N_TIMES(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_N_TIMES(writer, n, element_names::Level::Debug, dispatchAction, __VA_ARGS__)
#else
#   define CDEBUG_N_TIMES(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_DEBUG_LOG
#if ELPP_ERROR_LOG
#   define CERROR_N_TIMES(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_N_TIMES(writer, n, element_names::Level::Error, dispatchAction, __VA_ARGS__)
#else
#   define CERROR_N_TIMES(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_ERROR_LOG
#if ELPP_FATAL_LOG
#   define CFATAL_N_TIMES(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_N_TIMES(writer, n, element_names::Level::Fatal, dispatchAction, __VA_ARGS__)
#else
#   define CFATAL_N_TIMES(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_FATAL_LOG
#if ELPP_TRACE_LOG
#   define CTRACE_N_TIMES(writer, n, dispatchAction, ...)\
ELPP_WRITE_LOG_N_TIMES(writer, n, element_names::Level::Trace, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_N_TIMES(writer, n, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
...
#else
#   define CVERBOSE_N_TIMES(writer, n, vlevel, dispatchAction, ...) element_names::base::NullWriter()
#endif  // ELPP_VERBOSE_LOG
//
...
// Normal logs
#define CLOG(LEVEL, ...)\
C##LEVEL(element_names::base::Writer, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CVLOG(vlevel, ...) CVERBOSE(element_names::base::Writer, vlevel, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
// Conditional logs
#define CLOG_IF(condition, LEVEL, ...)\
C##LEVEL##_IF(element_names::base::Writer, condition, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CVLOG_IF(condition, vlevel, ...)\
CVERBOSE_IF(element_names::base::Writer, condition, vlevel, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
// Hit counts based logs
#define CLOG_EVERY_N(n, LEVEL, ...)\
C##LEVEL##_EVERY_N(element_names::base::Writer, n, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CVLOG_EVERY_N(n, vlevel, ...)\
CVERBOSE_EVERY_N(element_names::base::Writer, n, vlevel, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CLOG_AFTER_N(n, LEVEL, ...)\
C##LEVEL##_AFTER_N(element_names::base::Writer, n, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CVLOG_AFTER_N(n, vlevel, ...)\
CVERBOSE_AFTER_N(element_names::base::Writer, n, vlevel, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CLOG_N_TIMES(n, LEVEL, ...)\
C##LEVEL##_N_TIMES(element_names::base::Writer, n, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CVLOG_N_TIMES(n, vlevel, ...)\
CVERBOSE_N_TIMES(element_names::base::Writer, n, vlevel, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
//
...
#else
#   define ELPP_CURR_FILE_LOGGER_ID element_names::base::consts::kDefaultLoggerId
#endif
#undef ELPP_TRACE
...
#undef DPLOG_IF
#define CPLOG(LEVEL, ...)\
C##LEVEL(element_names::base::PErrorWriter, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define CPLOG_IF(condition, LEVEL, ...)\
C##LEVEL##_IF(element_names::base::PErrorWriter, condition, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define DCPLOG(LEVEL, ...)\
if (ELPP_DEBUG_LOG) C##LEVEL(element_names::base::PErrorWriter, element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
#define DCPLOG_IF(condition, LEVEL, ...)\
C##LEVEL##_IF(element_names::base::PErrorWriter, (ELPP_DEBUG_LOG) && (condition), element_names::base::DispatchAction::NormalLog, __VA_ARGS__)
...
C##LEVEL(element_names::base::Writer, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define CSYSLOG_IF(condition, LEVEL, ...)\
C##LEVEL##_IF(element_names::base::Writer, condition, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define CSYSLOG_EVERY_N(n, LEVEL, ...) C##LEVEL##_EVERY_N(element_names::base::Writer, n, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define CSYSLOG_AFTER_N(n, LEVEL, ...) C##LEVEL##_AFTER_N(element_names::base::Writer, n, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define CSYSLOG_N_TIMES(n, LEVEL, ...) C##LEVEL##_N_TIMES(element_names::base::Writer, n, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define SYSLOG(LEVEL) CSYSLOG(LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define SYSLOG_IF(condition, LEVEL) CSYSLOG_IF(condition, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define SYSLOG_EVERY_N(n, LEVEL) CSYSLOG_EVERY_N(n, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define SYSLOG_AFTER_N(n, LEVEL) CSYSLOG_AFTER_N(n, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define SYSLOG_N_TIMES(n, LEVEL) CSYSLOG_N_TIMES(n, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define DCSYSLOG(LEVEL, ...) if (ELPP_DEBUG_LOG) C##LEVEL(element_names::base::Writer, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define DCSYSLOG_IF(condition, LEVEL, ...)\
C##LEVEL##_IF(element_names::base::Writer, (ELPP_DEBUG_LOG) && (condition), element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define DCSYSLOG_EVERY_N(n, LEVEL, ...)\
if (ELPP_DEBUG_LOG) C##LEVEL##_EVERY_N(element_names::base::Writer, n, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define DCSYSLOG_AFTER_N(n, LEVEL, ...)\
if (ELPP_DEBUG_LOG) C##LEVEL##_AFTER_N(element_names::base::Writer, n, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define DCSYSLOG_N_TIMES(n, LEVEL, ...)\
if (ELPP_DEBUG_LOG) C##LEVEL##_EVERY_N(element_names::base::Writer, n, element_names::base::DispatchAction::SysLog, __VA_ARGS__)
#   define DSYSLOG(LEVEL) DCSYSLOG(LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define DSYSLOG_IF(condition, LEVEL) DCSYSLOG_IF(condition, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define DSYSLOG_EVERY_N(n, LEVEL) DCSYSLOG_EVERY_N(n, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define DSYSLOG_AFTER_N(n, LEVEL) DCSYSLOG_AFTER_N(n, LEVEL, element_names::base::consts::kSysLogLoggerId)
#   define DSYSLOG_N_TIMES(n, LEVEL) DCSYSLOG_N_TIMES(n, LEVEL, element_names::base::consts::kSysLogLoggerId)
#else
#   define CSYSLOG(LEVEL, ...) element_names::base::NullWriter()
#   define CSYSLOG_IF(condition, LEVEL, ...) element_names::base::NullWriter()
#   define CSYSLOG_EVERY_N(n, LEVEL, ...) element_names::base::NullWriter()
#   define CSYSLOG_AFTER_N(n, LEVEL, ...) element_names::base::NullWriter()
#   define CSYSLOG_N_TIMES(n, LEVEL, ...) element_names::base::NullWriter()
#   define SYSLOG(LEVEL) element_names::base::NullWriter()
#   define SYSLOG_IF(condition, LEVEL) element_names::base::NullWriter()
#   define SYSLOG_EVERY_N(n, LEVEL) element_names::base::NullWriter()
#   define SYSLOG_AFTER_N(n, LEVEL) element_names::base::NullWriter()
#   define SYSLOG_N_TIMES(n, LEVEL) element_names::base::NullWriter()
#   define DCSYSLOG(LEVEL, ...) element_names::base::NullWriter()
#   define DCSYSLOG_IF(condition, LEVEL, ...) element_names::base::NullWriter()
#   define DCSYSLOG_EVERY_N(n, LEVEL, ...) element_names::base::NullWriter()
#   define DCSYSLOG_AFTER_N(n, LEVEL, ...) element_names::base::NullWriter()
#   define DCSYSLOG_N_TIMES(n, LEVEL, ...) element_names::base::NullWriter()
#   define DSYSLOG(LEVEL) element_names::base::NullWriter()
#   define DSYSLOG_IF(condition, LEVEL) element_names::base::NullWriter()
#   define DSYSLOG_EVERY_N(n, LEVEL) element_names::base::NullWriter()
#   define DSYSLOG_AFTER_N(n, LEVEL) element_names::base::NullWriter()
#   define DSYSLOG_N_TIMES(n, LEVEL) element_names::base::NullWriter()
#endif  // defined(ELPP_SYSLOG)
//
...
#define CCHECK_STREQ(str1, str2, ...) CLOG_IF(!element_names::base::utils::Str::cStringEq(str1, str2), FATAL, __VA_ARGS__) \
<< "Check failed: [" << #str1 << " == " << #str2 << "] "
#define CCHECK_STRNE(str1, str2, ...) CLOG_IF(element_names::base::utils::Str::cStringEq(str1, str2), FATAL, __VA_ARGS__) \
<< "Check failed: [" << #str1 << " != " << #str2 << "] "
#define CCHECK_STRCASEEQ(str1, str2, ...) CLOG_IF(!element_names::base::utils::Str::cStringCaseEq(str1, str2), FATAL, __VA_ARGS__) \
<< "Check failed: [" << #str1 << " == " << #str2 << "] "
#define CCHECK_STRCASENE(str1, str2, ...) CLOG_IF(element_names::base::utils::Str::cStringCaseEq(str1, str2), FATAL, __VA_ARGS__) \
<< "Check failed: [" << #str1 << " != " << #str2 << "] "
...
namespace element_names { \
namespace base { \
element_names::base::type::StoragePointer elStorage(val); \
} \
element_names::base::debug::CrashHandler elCrashHandler(ELPP_USE_DEF_CRASH_HANDLER); \
}

#if ELPP_ASYNC_LOGGING
#   define INITIALIZE_EASYLOGGINGPP\
ELPP_INIT_EASYLOGGINGPP(new element_names::base::Storage(element_names::LogBuilderPtr(new element_names::base::DefaultLogBuilder()),\
new element_names::base::AsyncDispatchWorker()))\

#else
#   define INITIALIZE_EASYLOGGINGPP ELPP_INIT_EASYLOGGINGPP(new element_names::base::Storage(element_names::LogBuilderPtr(new element_names::base::DefaultLogBuilder())))
#endif  // ELPP_ASYNC_LOGGING
#define INITIALIZE_NULL_EASYLOGGINGPP\
ELPP_INITI_BASIC_DECLR\
namespace element_names {\
namespace base {\
element_names::base::type::StoragePointer elStorage;\
}\
element_names::base::debug::CrashHandler elCrashHandler(ELPP_USE_DEF_CRASH_HANDLER);\
}
...
namespace element_names {\
namespace base {\
element_names::base::type::StoragePointer elStorage(initializedStorage);\
}\
element_names::base::debug::CrashHandler elCrashHandler(ELPP_USE_DEF_CRASH_HANDLER);\
}

#if defined(ELPP_UNICODE)
#   define START_EASYLOGGINGPP(argc, argv) element_names::Helpers::setArgs(argc, argv); std::locale::global(std::locale(""))
#else
#   define START_EASYLOGGINGPP(argc, argv) element_names::Helpers::setArgs(argc, argv)
#endif  // defined(ELPP_UNICODE)
