...
7. **Запуск схемы**
  ...
  ```
	run
  ```
8. **Отчистка схемы** - удаление старой схемы и создание новой
```
	clear
```
