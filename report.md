...
1. Язык С++ поддерживает прямую передачу и move-семантику, которая активно используется в программе.
2. Существует ряд библиотек boost, использование которых сильно уменьшило объем кода позволило сократить время на написание аналогичных классов.
3. Существует библиотека cpp-expression-parcer, с помощью которой был разработан интерпретатор
...
Графическая составляющая проекта сделана в среде разработки Qt. Этот выбор был сделан по причине удобства и широкого функционала данной среды. Также, так как код программы написан на C++, его без проблем можно использовать в Qt.
