...
![scheme_wires](https://vk.com/im?peers=c97_267593012_181399916&sel=c135&z=photo41864547_456239116%2Fmail272734)
...
- `add(type - ключевое слово(тип элемента пример: xor), @name_of_element - имя элемента(может задаваться выражением, например: XOR[5], XOR[i]), num_of_in_pins - число, num_of_out_pins - число, mod<svector or pvector>(только для sbox-ов и p-block-ов) - mod - символ модификатора системы счисления(b - 2, d - 10, h - 16), <x, y> - координаты левого верхнего угла(может задаваться выражением), <width, height> - размеры элемента(может задаваться выражением))` - команда добавления элемента.  
- `connect(@name_of_sender - имя элемента, <out_pins_vector> - вектор выходных пинов(может задаваться выражением), @name_of_receiver - имя присоединяемого элемента, <in_pins_vector> - вектор входных пинов(может задаваться выражением))` - соединение пинов элементов  
- оператор `=` пример: `a = b` - пояснения не требуются  
 Далее - команды для криптоанализа:  
- `run_element(@name - имя элемента, <bitset_pool> - набор векторов(поддерживаются маски))` - прогон нескольких векторов через элемент  
- `run_scheme(<bitset_pool> - набор векторов(поддерживаются маски))` - прогон нескольких векторов через схему  
- `assembly` - сборка схемы
- `run` - запуск схемы
